...
rutas_usuarios = Blueprint('rutas_usuarios', 'rutas_usuarios')
...
def registro_usuarios():
    ...
    if request.method == 'POST':
        ...
        if accion == 'Registrar':
            ...
            campos = ['numero_documento', 'nombre_usuario', 'apellido_usuario',
                      'tipo_documento_usuario', 'correo_electronico_usuario',
                      'telefono', 'id_rol', 'contrasena']
            ...
            if all(request.form.get(field) for field in campos):
                ...
                contrasena = hashlib.sha256(contrasena_plana.encode('utf-8')).hexdigest()

                conn = obtener_conexion()
                cur = conn.cursor(pymysql.cursors.DictCursor)

                cur.execute(
                    'SELECT * FROM usuarios WHERE correo_electronico_usuario = %s OR numero_documento = %s',
                    (correo_electronico_usuario, numero_documento)
                )
                usuario = cur.fetchone()

                if usuario:
                    text = 'El correo o número de documento ya se encuentra registrado'
                else:
                    cur.execute('''
                        INSERT INTO usuarios 
                        (numero_documento, nombre_usuario, apellido_usuario, tipo_documento_usuario, 
                         correo_electronico_usuario, telefono, id_rol, contrasena)
                        VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                    ''', (
                        numero_documento, nombre_usuario, apellido_usuario, tipo_documento_usuario,
                        correo_electronico_usuario, telefono, rol, contrasena
                    ))
                    conn.commit()
...
def listar_usuarios():
    ...
    cur.execute('SELECT COUNT(*) as total FROM usuarios_por_rol')
    total_records = cur.fetchone()['total']
    total_pages = (total_records + per_page - 1) // per_page

    cur.execute('SELECT * FROM usuarios_por_rol LIMIT %s OFFSET %s', (per_page, offset))
    ...
    return render_template(
        'crud_usuarios/listar_usuarios.html',
        usuarios=usuarios, text=text, textM=textM, textE=textE,
